...
export class HttpService extends HttpAdapter {
  ...
  public get<T>(url: string): Observable<T> {
    return this.http
      .get<T>(url)
      .pipe(
        catchError((err: HttpErrorResponse) => this.handleError('get', err))
      );
  }
  ...
  public put<T>(url: string, body?: unknown): Observable<T> {
    return this.http
      .put<T>(url, body)
      .pipe(
        catchError((err: HttpErrorResponse) => this.handleError('put', err))
      );
  }
  ...
  public post<T>(url: string, body?: unknown): Observable<T> {
    return this.http
      .post<T>(url, body)
      .pipe(
        catchError((err: HttpErrorResponse) => this.handleError('post', err))
      );
  }
